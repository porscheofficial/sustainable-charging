...
def main():
    ...
    n_weather_datapoints = (
        data_req.weather_data_lookback + data_req.weather_data_lookahead
    )
    random_smard_data = pd.DataFrame(
        np.random.rand(n_smard_datapoints, len(data_req.smard_data_columns)).astype(
            np.float32
        ),
        columns=data_req.smard_data_columns,
    )
    random_weather_data = pd.DataFrame(
        np.random.rand(n_weather_datapoints, len(data_req.weather_data_columns)).astype(
            np.float32
        ),
    ...
    random_smard_data["timestamp"] = pd.date_range(
        start=now - pd.Timedelta(hours=n_smard_datapoints),
        periods=n_smard_datapoints,
        freq="H",
    )
