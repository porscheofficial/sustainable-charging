...
from time_series_model.evaluation import get_covariate_args, cross_validation_without_refit, co2_rmse
...
def load_smard_data():
    ...
    return smard_dataloader.get_dataframe('train'), smard_dataloader.get_dataframe('validation'), smard_dataloader.get_dataframe('test')
...
def fit_model(model: VARIMA, train: TimeSeries, covariates: TimeSeries, save_model: bool = False):
...
def main():
    ...
    logger.info(f'Train start: {adapted_train.start_time()}, Train end: {adapted_train.end_time()}')
    if WEATHER_INCLUDED:
    ...
    study.optimize(lambda trial: objective(trial, train=adapted_train, validation=validation, covariates=covariates), n_trials=1)
    ...
    if WEATHER_INCLUDED:
        test_covariates = covariates_time.stack(weather_data)[final_train.start_time():]
    else:
